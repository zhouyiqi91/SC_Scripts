...
# filter
print (paste("Raw cell: ",all_data@raw.data@Dim[2]))

all_data <- FilterCells(object = all_data, subset.names = c("percent.mito"), low.thresholds = c(-Inf), high.thresholds = c(mtmax))
print (paste("Cell after filter percent.mito: ",all_data@data@Dim[2]))

all_data <- FilterCells(object = all_data, subset.names = c("nGene"), low.thresholds = c(genemin), high.thresholds = c(genemax))
print (paste("Cell after filter nGene: ",all_data@data@Dim[2]))

print (paste("Remain cell Percent: ",all_data@data@Dim[2]/all_data@raw.data@Dim[2]))
...
print (TSNEPlot(all_data,do.label=TRUE,do.return=TRUE,pt.size=0.5)+theme(legend.position = "bottom"))
dev.off()
